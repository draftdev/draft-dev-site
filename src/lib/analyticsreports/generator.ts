...
const DEFAULT_CHARTS = [
  ...
  { metric: 'Citation Consistency', type: 'line', title: 'Citation Consistency', showPrevious: false, description: 'Percentage of responses that cited the source out of all responses citing any source', size: 'medium' },
]
...
function buildCharts(metrics: any[], chartConfig: typeof DEFAULT_CHARTS, brandColor: string) {
  const renderer = new ChartRenderer({ brandColor })
  const charts: { id: string; title: string; config: unknown; description?: string; size: string }[] = []

  for (const cfg of chartConfig) {
    const chartData = getChartData(metrics, cfg.metric)
    if (chartData.values.length === 0) continue

    const data = {
      labels: chartData.labels,
      values: chartData.values,
      previousValues: cfg.showPrevious ? chartData.previousValues : null,
    ...
    let config
    if (cfg.type === 'bar') config = renderer.createBarChart(data, cfg.title)
    else if (cfg.type === 'comparison') config = renderer.createComparisonChart(data, cfg.title)
    else config = renderer.createLineChart(data, cfg.title)
...
export async function generateReport(opts: GenerateReportOptions): Promise<string> {
  const { excelBuffer, month, clientName, logoBuffer, logoExt } = opts

  const excelData = await parseExcel(excelBuffer, month)
  // eslint-disable-next-line prefer-const
  let { metrics, notes } = excelData

  // Inline note from Key Metrics sheet
  const metricsWithNote = metrics as typeof metrics & { _inlineNote?: string }
  if (metricsWithNote._inlineNote) {
    notes.unshift({
      date: `${month}-01`,
      title: 'Monthly Summary',
      note: metricsWithNote._inlineNote,
      section: 'highlights',
      screenshotFile: null,
    })
    delete metricsWithNote._inlineNote
  }

  // Filter notes to the requested month
  const monthNotes = notes.filter(n => !n.date || n.date.startsWith(month))

  // Build screenshots map from embedded Excel images
  ...
  // Current-month metrics for the summary grid
  const currentMonthMetrics = metrics.filter(
    m => m.isCurrentMonth === true || !('isCurrentMonth' in m)
  )
  const metricsByCategory = groupMetricsByCategory(currentMonthMetrics)

  // Charts use full historical data
  const charts = buildCharts(
    metrics,
    DEFAULT_CONFIG.charts,
  ...
  if (logoBuffer && logoBuffer.length > 0) {
    const mime =
      logoExt === 'svg' ? 'image/svg+xml' :
      logoExt === 'jpg' || logoExt === 'jpeg' ? 'image/jpeg' :
      `image/${logoExt}`
    clientLogo = `data:${mime};base64,${logoBuffer.toString('base64')}`
  }

  const templateData = {
    clientName,
    brandColor: DEFAULT_CONFIG.brandColor,
  ...
  return ejs.render(REPORT_TEMPLATE, templateData)
}
